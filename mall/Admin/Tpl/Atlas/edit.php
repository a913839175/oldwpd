...
//ajax判断是否为最终分类
	...
	function ajx(){
			...
			//ajax显示获取自定义字段
			$.get('<{:U("Content/edit_custom_ajax")}>',{module_id:8,id:id,cid:cid,rand:Math.random()},function(data){
...
//判断名称是否唯一	
	function ajxname(){
		...
		//ajax验证
		$.get('<{:U("Atlas/ajaxname")}>',{atlname:atlname,id:editid,rand:Math.random()},function(data){
			$("#valu1").html(data);
		})
	}
...
$(function(){
	...
	$(".oneedit").live("click",function(){
		...
		$("input[name='pichref1']").val($("#i_pichref_"+id).val()); //将隐藏域的值赋给图片名称
...
<div class="wrap J_check_wrap">
  ...
  <ul class="cc">
        ...
        <li><a href="javascript:void(0)">修改图册</a></li>
      </ul>
	</div>
  <div class="h_a">图册内容</div>
  <form name="myform" action="<{:U("Atlas/edit")}>" method="post" class="J_ajaxForm" enctype="multipart/form-data">
    <div class="table_full"> 
    <table width="100%" class="table_form contentWrap">
        <tbody> 
        <tr>
          <th width="100">图册名称<font color="#FF0000">*</font></th>
          <td><input type="text" name="atlname" value="<{$data.atlname}>" class="input input_hd J_title_color" id="atlname" size="70">&nbsp;</td>
        </tr>
        <tr>
        ...
        <tr>
          <th>图册简介</th>
          <td><textarea name="atldes"  class="inputtext" style=" width:450px;height:100px;" placeholder="输入图册介绍："><{$data.atldes}></textarea></td>
        </tr>
        <tr>
        	<th>上传图片</th>
            <td>
            ...
            <td>
                ...
                <foreach name="datapic" item="vo">
					...
					?>
                		...
                		<span class="quanbu" id="bufen_<{$vo.id}>"><span id="onoepic_<{$vo.id}>" class="ab" ><a href="javascript:void(0)"><img height="100px" width="120px" src="__PUBLIC__/Uploads/Atl/s_<{$vo.savename}>"  class="onepic" /></a></span><span id="caozuo_<{$vo.id}>" ><a href="javascript:void(0)" class="onedel" dataid="<{$vo.id}>" title="删除">删除</a>/<a href="javascript:void(0)" title="编辑" class="oneedit" dataid="<{$vo.id}>">编辑</a></span>&nbsp;&nbsp;&nbsp;&nbsp;</span>
                </foreach>
        ...
        <tr>
            ...
            <td><input type="text" class="input" size="60" name="picname1" placeholder="输入该图片名称：" value="<{$dataonepic.picname}>" dataid="<{$dataonepic.id}>"/></td>
            
        </tr>
        
        <tr>
        	<th>图片描述</th>
            <td><textarea style="width:400px; height:80px;" placeholder="输入该图片描述：" name="picdes1" id="picdes1"><{$dataonepic.picdes}></textarea></td>
          
        </tr>
        
        <tr>
        	<th>图片链接</th>
            <td><input type="text" class="input" size="30"  name="pichref1" value="<{$dataonepic.pichref}>" /></td>
            
        </tr>

        <tr>
        	<th>图片排序</th>
            <td><input type="text" class="input" size="30"  name="picorderby1" value="<{$dataonepic.picorderby}>" /></td>
     ...
     <div class="btn_wrap" style="z-index:999;">
      <div class="btn_wrap_pd">             
        ...
        <foreach name="datapic" item="vo">
        	<input type="hidden" name="picname[]" class="c_picname" id="i_picname_<{$vo.id}>" value="<{$vo.picname}>" />
            <input type="hidden" name="savename[]" class="c_savename" id="i_savename_<{$vo.id}>" value="<{$vo.savename}>" />
            <input type="hidden" name="picdes[]" class="c_picdes" id="i_picdes_<{$vo.id}>" value="<{$vo.picdes}>" />
            <input type="hidden" name="pichref[]" class="c_pichref" id="i_pichref_<{$vo.id}>" value="<{$vo.pichref}>" />
            <input type="hidden" name="picorderby[]" class="c_picorder" id="i_picorderby_<{$vo.id}>" value="<{$vo.picorderby}>">
        </foreach>
