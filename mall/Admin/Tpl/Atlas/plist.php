			...
			if(piliang==1){
				...
				art.dialog({
					...
					ok:function(){
						...
						$.get('<{:U("Atlas/plshow")}>',{str:str,ran:Math.random()},function(data){
							if(data==1){
								art.dialog({
								title:"提示",
								icon:"succeed",
				...
				art.dialog({
					...
					ok:function(){
						...
						$.get('<{:U("Atlas/plhidden")}>',{str:str,ran:Math.random()},function(data){
							if(data==1){
								art.dialog({
								title:"提示",
								icon:"succeed",
				...
				art.dialog({
					...
					ok:function(){
						...
						$.get('<{:U("Atlas/pldelete")}>',{str:str,ran:Math.random()},function(data){
							if(data==1){
								art.dialog({
								title:"提示",
								icon:"succeed",
	...
	$(function(){
		$(".view").click(function(){
			...
			Wind.use("artDialog", function () {
					...
					art.dialog.open("__APP__/Atlas/view/id/"+id, {width: 800, height: 600,title:'图片预览-'+name,background:"#CCCCCC",opacity:0.8,lock:true,id:'abe',drag:false,resize:false,
									cancelVal:"关闭",
									});
				});
		})	   
...
<div class="wrap J_check_wrap">
  ...
  <ul class="cc">
        ...
        <li><a href='<{:U("Atlas/plist")}>'>图册列表</a></li>
      </ul>
	</div>
<div class="mb10">
        <a href='<{:U("Atlas/add")}>' class="btn" title="添加图册" ><span class="add"></span>添加图册</a>
  </div>

  <div class="h_a">搜索</div>
  <form name="form1" action="<{:U("Atlas/plist")}>" method="post" >
    <div class="search_type cc mb10">
      <div class="mb10"> <span class="mr20"> 搜索类型：
        <select name="searchtype">
          <option value="1" <if condition="$_GET['searchtype'] eq '1'">selected</if> >图册名称</option>
        </select>
        关键字：
      ...
      <table width="100%" cellspacing="0">
        <thead>
          <tr>
            ...
            <td width="314" align="left">图册名称</td>
            <td width="152" align="center">所属分类</td>
            <td width="100" align="center">图片数目</td>
        ...
        <tbody>
          <volist name="data" id="vo">
            <tr>
              ...
              <td align="left"><{$vo.atlname}></td>
              <td align="center"><{$vo.atlclassname}></td>
               <td align="center">
              	<a href="javascript:void(0)" class="view" dataid="<{$vo.id}>" dataname="<{$vo.atlname}>"><{$vo.num}></a>
               </td>
              ...
              <td width="263" align="center"><a href="<{:U("Atlas/edit",array("id"=>$vo['id']))}>">修改</a> | <a class="J_ajax_del" href="<{:U("Atlas/delete",array("id"=>$vo['id']))}>">删除</a> | 
              <if condition="$vo.isshow eq 1">
              <a href="<{:U("Atlas/isshow",array("id"=>$vo['id'],"isshow"=>"0"))}>">隐藏</a>
              <else />
              <a style="color:red;" href="<{:U("Atlas/isshow",array("id"=>$vo['id'],"isshow"=>"1"))}>">显示</a>
              </if>
