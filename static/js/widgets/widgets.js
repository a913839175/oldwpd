...
d.extend(e.prototype,{
    init:function(){
        var a=this,b=this._ui;
        return b.img.click(function(){
            var c=(new Date).getTime();
            b.img.attr("src","/?plugins&q=imgcode?ts="+c),b.input.val(""),a.clean()
...
d.extend(e.prototype,{
    _getDelta:function(){
        var a=null;
        if(this.delta||this.start&&this.end)if(this.delta)a=this.delta;
            else{
            var b=this.start.split("-"),c=this.end.split("-");
        ...
        a/=1e3;
        var d=Math.floor(a/86400),e=Math.floor(a/3600),f=Math.floor((a-3600*e)/60),g=Math.floor(a-3600*e-60*f);
        return{
            days:d,
            hours:e,
            minutes:f,
...
init:function(){
    ...
    var b=this._getDelta(),c=0,e=null,f=this;
...
define("widgets/form",["jquery","validate","ui-poptip"],function(a,b,c){
    ...
    e.err={
        ...
        bankCode:"请选择充值银行",
        bankCardNo:"请输入银行卡号",
        bankLeavePhone:"请输入银行预留手机号",
        isbankLeavePhone:"请正确输入您的银行预留手机号",
        vcode:"请输入验证码",
        ...
        isIntNum:"请输入正整数",
        codeLengthfour:"请输入4位验证码",
        codeLength:"请输入6位验证码",
        minCachLength:"提现金额不能小于100元",
//        maxCachLength:"单笔提现金额5万，每日提现金额50万",
    ...
    e.checkCode=function(a){
        a.ele.rules("add",{
            remote:{
                url:a.url||"/?user&q=login&q=check_randCodes",
                data:a.data,
                dataType:"JSON",
                dataFilter:function(b){
                    var c=b;
                    if("true"==c)return a.success&&"function"==typeof a.success&&a.success.call(this,b),!0;
                    var e=d("#code-label"),f='<label class="isMaxCodeNum error">'+c.message+"</label>";
                    return null!=c.message?(e.parent(".fn-hide").length||e.wrap('<div class="fn-hide">'),d(".isMaxCodeNum").length?d(".isMaxCodeNum").show():e.parent("div").after(f)):(e.parent(".fn-hide").length&&e.unwrap('<div class="fn-hide">'),d(".isMaxCodeNum").hide()),a.failed&&"function"==typeof a.failed&&a.failed.call(this,b),!1
                    }
                },
        messages:{
    ...
    rules:{
        register:{
            username:{
                required:!0,
                isMobile:!0,
                remote:"/?user&q=login&q=check_username"
            },
            password:{
                required:!0,
                minlength:6,
...
emailUpdateByOldStepOne:{
    code:{
        required:!0,
        remote:{
            url:"/?user&q=login&q=check_randCodes",
            dataType:"JSON",
...
mobileCodeForReg:{
    mobileCode:{
        required:e.err.required,
        minlength:e.err.codeLengthfour,
        maxlength:e.err.codeLengthfour
        }
    },
login:{
    j_password:{
...
loaninfo:{
    ...
    loanProductType:{
        required:e.err.loanProductType
        }
    },
borrowerinfo:{
    realName:{
...
recharge:{
    ...
    bankCode:{
        required:e.err.bankCode
        },
    userBankId:{
        required:e.err.bankCode
        },
    bankCardNo:{
        required:e.err.bankCardNo
        },
    bankLeavePhone:{
        required:e.err.bankLeavePhone
        },
    vcode:{
        required:e.err.vcode
        }
    },
withdraw:{
    amount:{
        ...
        max:e.err.maxCachLength,
        maxlength:e.err.maxNumberLength,
        isEnough:e.err.isEnough
        },
    cashPassword:{
        required:e.err.required,
        maxlength:e.err.maxCashPasswordLength,
        isPassWord:e.err.isPassWord
        },
    userBankId:{
...
addcard:{
    ...
    deposit:{
        required:e.err.required,
        maxlength:e.err.depositLength,
        isCardDeposit:e.err.isCardDeposit
        },
    cardId:{
        required:e.err.required,
    ...
    validateCode:{
        required:e.err.phoneCodeMsg,
        minlength:e.err.phoneCodeMsgLength,
        maxlength:e.err.phoneCodeMsgLength
        }
    },
modpsw:{
    oldPassword:{
...
setId:{
    ...
    blPhone:{
        required:e.err.required,
        isMobile:e.err.isbankLeavePhone
    },
    blNo:{
        required:e.err.required,
        isBankCard:e.err.isBankCard
...
borrowsq:{
    ...
    function c(){
        function c(a){
            ...
            var e=a.attr2("target"),i=a.attr2("action");
        ...
        var m=[];
        try{
            if(h.extraData)for(var n in h.extraData)h.extraData.hasOwnProperty(n)&&m.push(d.isPlainObject(h.extraData[n])&&h.extraData[n].hasOwnProperty("name")&&h.extraData[n].hasOwnProperty("value")?d('<input type="hidden" name="'+h.extraData[n].name+'">').val(h.extraData[n].value).appendTo(r)[0]:d('<input type="hidden" name="'+n+'">').val(h.extraData[n]).appendTo(r)[0]);h.iframeTarget||(k.appendTo("body"),l.attachEvent?l.attachEvent("onload",g):l.addEventListener("load",g,!1)),setTimeout(b,15);
            try{
                r.submit()
                }catch(o){
    ...
    if(!m.aborted&&!z){
        ...
        if(y&&y.location.href!=h.iframeSrc||p){
            ...
            try{
                ...
                var n=(h.dataType||"").toLowerCase(),o=/(json|script|text)/.test(n);
                if(o||h.textarea){
                    ...
                    if(r)m.responseText=r.value,m.status=Number(r.getAttribute("status"))||m.status,m.statusText=r.getAttribute("statusText")||m.statusText;
                        ...
                        var v=y.getElementsByTagName("pre")[0],w=y.getElementsByTagName("body")[0];
...
var h,i,j,k,l,m,n,o,p,q,r=a[0],s=d.Deferred();
...
var t=1,u=2,v=d("meta[name=csrf-token]").attr("content"),w=d("meta[name=csrf-param]").attr("content");
w&&v&&(h.extraData=h.extraData||{},h.extraData[w]=v),h.forceSync?e():setTimeout(e,10);
var x,y,z,A=50,B=d.parseXML||function(a,b){
    return window.ActiveXObject?(b=new ActiveXObject("Microsoft.XMLDOM"),b.async="false",b.loadXML(a)):b=(new DOMParser).parseFromString(a,"text/xml"),b&&b.documentElement&&"parsererror"!=b.documentElement.nodeName?b:null
    },C=d.parseJSON||function(a){
    return window.eval("("+a+")")
    },D=function(a,b,c){
    var e=a.getResponseHeader("content-type")||"",f="xml"===b||!b&&e.indexOf("xml")>=0,g=f?a.responseXML:a.responseText;
...
if(a.length&&(!b||"object"==d.type(b))){
    ...
    var l=a.find("input[type=file]"),m=l.length>0;
    b=d.extend({
        url:h,
        data:i,
        beforeSend:function(){
    ...
    isUserName:function(a){
        var b=/^\w+(@)\w+(\.\w+)(\.\w+|)$/,c=/^(((13[0-9]{1})|(14[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/;
    ...
    is0Mobile:function(a){
        return/^01[0-9]{10}$/.test(a)
    },
    isPhone:function(a){
    ...
    isMobileOrEmail:function(a){
        return this.isMobile(a)||this.isEmail(a)||this.is0Mobile(a)
        },
    isUrl:function(a){
    ...
    ,isAmount:function(a){return/^[0-9]*(\.[0-9]{1,2})?$/.test(a)}
    ,isPostCode:function(a){return/^\d{6}$/.test(a)}
    ,isBankCard:function(a){return/^\d{12,19}$/.test(a)}
    ,isCardDeposit:function(a){return/^[a-zA-Z0-9\(\)\（\）\u4e00-\u9fa5]+$/.test(a)}
    ,isEducationCode:function(a){return/^\d{12}$/.test(a)}
    ,isIntNum:function(a){return/^\d+$/.test(a)}
    ,isOneDecimal:function(a){return/^(([1-9]{1}\d*)|([0]{1}))(\.(\d){1})?$/.test(a)}
    ,isLuhn:function(a){if(/[^0-9-\s]+/.test(a))return!1;var b=0,c=0,d=!1;a=a.replace(/\D/g,"");for(var e=a.length-1;e>=0;e--){var f=a.charAt(e);c=parseInt(f,10),d&&(c*=2)>9&&(c-=9),b+=c,d=!d}return b%10===0}},e.isEmptyObj=function(a){var b=!0;if("[object Array]"===Object.prototype.toString.call(a))b=!a.length;else{a=new Object(a);for(var c in a)return!1}return b},e.comma=function(a,b){var c=a;return(!b||1>b)&&(b=3),c=String(c).split("."),c[0]=c[0].replace(new RegExp("(\\d)(?=(\\d{"+b+"})+$)","ig"),"$1,"),c.join(".")},d.extend({uniqueArray:function(a,b){var c,d,e=a.length,f=a.slice(0);for("function"!=typeof b&&(b=function(a,b){return a===b});--e>0;)for(d=f[e],c=e;c--;)if(b(d,f[c])){f.splice(e,1);break}return f}}),e.addValidateMethod=function(a){"array"==d.type(a)&&d.each(a,function(a,b){jQuery.validator.addMethod(b,function(a,c){return this.optional(c)||e.is[b](a)})})},e.placeholder=function(){d(".ui-form").on("click",".placeholder",function(){d(this).hide().parent().children("input").trigger("focus")}).on("focus","input",function(){d(this).parent().children(".placeholder").hide()}).on("blur","input",function(){this.value||d(this).parent().children(".placeholder").show()}).find("input").trigger("focus")},e.randImage=function(a){var b=d("undefined"==typeof a?"#randImage":a);b.length>0&&b.click(function(){d(this).attr("src","/?plugins&q=imgcode?"+Math.random())})},e.tipfocus=function(a,b){var c=a[0].offsetWidth+a.parent().children(".ui-label").width()+10,e=2;
    ...
    function e(){
        ...
        var f=d("#"+a),g=d("#"+b),h=d("#"+c),i=d("#"+c).val();
    ...
    function g(){
        var a=l.length?l.val():"",b=ranco.length?ranco.val():"";
    ...
    var i,j=60,k=j,l=d("undefined"==typeof a?"#phone":"#"+a),ranco=d("#randCode");
b="undefined"==typeof b?"getMobileCode":b;
var m=d("#"+b),n=m.html(),o="undefined"==typeof c?"/sendPhoneCode.action?phone=":c,p="&randCode=";
;
return f.unDisabled?m.bind("click",function(){
    "disabled"!=d(this).attr("disabled")&&(f&&f.onStart&&d.isFunction(f.onStart)&&f.onStart(),g())
    }):m.removeAttr("disabled").html(n).unbind("click").bind("click",function(a){
    ...
    this.prepareForm();
    for(var a=0,b=this.currentElements=this.elements();b[a];a++)if(this.findByName(b[a].name).length&&this.findByName(b[a].name).length>1)for(var c=0;c<this.findByName(b[a].name).length;c++)this.check(this.findByName(b[a].name)[c]);else this.check(b[a]);return this.valid()
    },function(a){
    if(document.all&&!document.querySelector&&a.fn.validate){
        var b=a.fn.validate,c=Array.prototype.slice;
        a.fn.validate=function(){
            var a=c.call(arguments,0),d=this.attr;
...
var f={
    ...
    highlight:function(a){
        ...
        var b=d(a).siblings(".icon"),c=b.attr("class");
        if(c&&/gray/.test(c)){
...
var j=function(a,b,c){
    ...
    for(e;f>e;e++)for(var g in a)if(b[e].name==g){
...
if("array"==d.type(c.fieldArr)&&b.find("input[type='text'],input[type='password'],input[type='checkbox'],input[type='radio'],input[type='file'],select,textarea").each(function(){
    var a=d(this),b=a.data("is"),f=a.attr("name"),g={};
    
    if(b&&(b=d.trim(b).split(/\s+/)),b&&b.length)for(var h=0,i=b.length;i>h;h++)c.validateMethodArr.push(b[h]);
...
define("widgets/list",["jquery","common","protocol","simplePagination"],function(a,b,c){
    var d=a("jquery"),e=a("common"),f=a("protocol");
    ...
    d.extend(g.prototype,{
        ...
        getParams:function(){
            var a=this,b=arguments.callee.getRowValue;
            ...
            for(var c,e=function(){
                var b=[],c=a.container.find("ul.category");
                return c.each(function(){
                    b.push(d(this).data("category"))
                    }),b
                }(),f=0,g={};
                f<e.length;f++)c=b(d("input[name="+e[f]+"][checked=checked]")),""!==c&&(g[e[f]]=c);
            return g
            },
        loadStatus:function(){
            var a=this.getParams(),b=d.param(a);
        ...
        initTags:function(){
            ...
            d(a.categoryTag).click(function(){
                var b=d(this),c=b.siblings("li.all");
...
d.extend(h.prototype,{
    ...
    update:function(){
        var a=this,b=this._ui,c=d(this.id);
        b.sortable=c.children(".ui-list-title-sortable"),b.filterable=c.children(".ui-list-title-filterable"),this.render(),b.sortable.click(function(){
            var b=d(this),c=b.data("name"),e=a._status.sortable;
...
d.extend(i.prototype,{
    ...
    update:function(a,b){
        if(b!=this._count||a!=this._index){
            if(this.container.empty(),this._count=b,this._index=a,1>=b)return;
            var c=this;
...
add:function(a,b,c){
    var d=c||this.templateForItem,f=e.fillTemplate({
        data:a,
        template:d
    });
    if(b){
...
stopLoading:function(){
    ...
    var d=function(a){
        var b,c=a.match(/\d/g),d=a.match(/[a-z]/g),e=a.match(/[A-Z]/g),f=a.match(/[!%#$\^*~.&]/g),g=a.length,h=c?c.length:0,i=d?d.length:0,j=e?e.length:0,k=f?f.length:0,l=!1,m=!1,n=!1,o=!1,p=0,q={};
    ...
    var d=a("jquery"),e=function(a){
        function b(){
            k.each(function(a){
                var b=a+1,c=k.eq(a).data("background");
                c=c.replace(/url\(/,"").replace(/\) no-repeat 50\% 50\%/,""),d('<li><a href="#">'+b+"</a></li>").appendTo(n),m.push(c)
                }),k.eq(l).show(),a.target.parent().css("background","none"),n.find("li").eq(l).addClass(a.curClass)
            }
            function c(){
            d(a.target).find("li").hover(function(){
                clearInterval(j)
                },f),n.find("li").on("click",function(b){
                l=d(this).index(),g(l),d(this).addClass(a.curClass).siblings().removeClass(a.curClass),k.eq(l).fadeIn("slow").siblings().fadeOut("slow"),b.preventDefault(),clearInterval(j),e(l),f()
                })
            }
            function e(b){
            g(b),a.targetNav.find("li").eq(b).addClass(a.curClass).siblings().removeClass(a.curClass),k.eq(b).fadeIn("slow").siblings().fadeOut("slow")
            }
            function f(){
            j=setInterval(function(){
                e(l),l++,l>=n.find("li").length&&(l=0),g(l),k.eq(l).fadeIn().siblings().fadeOut(),n.find("li").eq(l).addClass(a.curClass).siblings().removeClass(a.curClass)
                },a.delay)
            }
            function g(a){
            var b=k.eq(a);
            b.data("isLoadImg")||b.data("isLoadImg",!0).css("background","url("+m[a]+") no-repeat 50% 50%")
            }
            function h(a){
            if(a++,a!=k.length+1){
                var b=new Image;
                b.src=m[a-1],b.onload=function(){
                    h(a)
                    }
                }
        }
...
var j,k=a.target.find("li"),l=(k.length,a.curIndex),m=[],n=d("<ul>").addClass("slider-nav-pointer").appendTo(a.targetNav);
    i()
    };
    ...
    d.extend(e.prototype,{
        init:function(){
            ...
            this.tabItems.click(function(){
                var b=a._current,c=a.getName(d(this));
