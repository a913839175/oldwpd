...
$_G['nowtime'] = time();
//渠道获取开始
$channel_h = parse_url($_SERVER['HTTP_REFERER']); 
if($channel_h['host']!=$_SERVER['SERVER_NAME'] && $channel_h['host']!=""){
    $beginToday=mktime(0,0,0,date('m'),date('d'),date('Y'));
    $endToday=mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
    $sql="select count(1) as 'num' from hrcf_channel where `phpsessid_s`='{$_COOKIE['PHPSESSID']}' and `addtime` between {$beginToday} and {$endToday}";
    $num=$mysql->db_fetch_array($sql);
    if($num['num']==0){
        $sql="insert into hrcf_channel set `referer_s`='{$channel_h['host']}',`phpsessid_s`='{$_COOKIE['PHPSESSID']}',`addtime`='".time()."'";
        $mysql->db_query($sql);
    }
}
//渠道获取结束
...
//IP限制，用于正式服测试
//var_dump(ip_address());
//if(!in_array(ip_address(), array('203.156.202.251'))){
//    echo "<script>window.location.href='/defend.htm';</script>";
//    exit();
//}
...
$system['sina_sign_type'] = "RSA";
$system['sina_pay_payee_identity_id'] = "hrcf5140114";
$_G['system'] = $system;
$_G['_system'] = $_system;
...
if ($_G["user_id"]!=""){
	...
	$_G["user_info"] = usersClass::GetUsers(array("user_id"=>$_G["user_id"]));//获取用户信息
...
foreach ($_G['site'] as $key => $value){
	...
	if ($value['pid'] == 9){
		if ($value['status']==1){
			$_G['site_sub_list_9'][] = $value;//子站点列表
		}
	}
	
	if ($value['pid'] == 28){
		if ($value['status']==1){
			$_G['site_sub_list_28'][] = $value;//子站点列表
		}
	}
...
elseif ($_G['query_site'] == "api" ){
	require_once ("modules/operate/index.php");exit;
}
//用户中心
elseif ($_G['query_site'] == "home" ){
	...
	exit;
		...
		**/
		...
		/* ************************author wangjie(top)**************************** */
		elseif($_G['query_site'] == "wxuser"){
			include_once ("modules/wx_member/index.php");
		}
		/* ************************author wangjie(bottom)********************** */
